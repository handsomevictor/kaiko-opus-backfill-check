...
def process_opus_config_for_dates():
    opus_dates_summary = pd.DataFrame(['year', 'season', 'start_time', 'end_time'])

    # iterate all config files in database/opus_config
    for root, dirs, files in os.walk(os.path.join(os.getcwd(), 'database', 'opus_config')):
        files.sort()
        for file in files:
            ...
            start_time = list(set(df['start_time'].tolist()))
            assert len(start_time) == 1
            end_time = list(set(df['end_time'].tolist()))
            assert len(end_time) == 1

            file_name = file.split('.')[0]
            file_year, file_season = file_name.split('_')

            tmp_df = pd.DataFrame([file_year,
                                      file_season,
                                      start_time[0],
                                      end_time[0]])

            opus_dates_summary = pd.concat([opus_dates_summary, tmp_df], axis=1)
    opus_dates_summary.to_csv(os.path.join(os.getcwd(), 'database', 'opus_dates_processed.csv'), index=False)
